...
def get_important(df: pl.LazyFrame, ticker: str) -> pl.LazyFrame:
    ...
    post_pmhigh = get_post_high(pm_hours, pmhigh_time)
    print(post_pmhigh.collect())
...
def check_combined_intra(combined_intra_path: str, date: datetime.date, ticker: str) -> bool:
    saved = (pl.scan_parquet(combined_intra_path)
          .select(pl.col(['date', 'ticker']))
          .collect()
          .rows()
        )
    return (date, ticker) in saved

def add_combined_intra(api_key: str, combined_intra_path: str, date: datetime.date, ticker: str):
    if check_combined_intra(combined_intra_path, date, ticker) is True:
        raise Exception(f'{ticker} on {date} is already in the dataframe!')

    df = get_polygon_data(api_key, date, ticker)
    df = format_df(df)
    imp_df = get_important(df, ticker=ticker)
    combined_df = pl.read_parquet(combined_intra_path)
    combined_df = combined_df.vstack(imp_df.collect())
    combined_df.write_parquet(combined_intra_path)

def remove_combined_intra(combined_intra_path: str, date: datetime.date, ticker: str):
    combined_df = (pl.scan_parquet(combined_intra_path)
                   .filter(
                       pl.col('date').eq(date) &
                       pl.col('ticker').eq(ticker)
                       ))
    combined_df.collect().write_parquet(combined_intra_path)
